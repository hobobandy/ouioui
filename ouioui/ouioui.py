...
class Ouioui:
    def __init__(self, config: OUIConfig):
        ...
        logging.basicConfig(
            format="%(asctime)s: %(message)s", level=self.config.logging_level, datefmt="%H:%M:%S"
        )
    ...
    def update(self):
        ...
        for prefix, manuf in custom_set:
            if prefix in ieee_set:
                logger.info(f"Custom OUI - Overriding {prefix} `{ieee_set[prefix].strip()}` with `{manuf.strip()}`.")
            else:
                logger.info(f"Custom OUI - Adding {prefix} `{manuf}` from custom OUI list.")
    ...
    def update_custom(self):
        ...
        for prefix, manuf in custom_set:
            if prefix in ieee_set:
                logger.info(f"Overriding {prefix} `{ieee_set[prefix].strip()}` with `{manuf.strip()}`.")
            else:
    ...
    def package_for_kismet(self):
        ...
        final_set = set(f"{prefix}\t{manuf.strip()}" for prefix, manuf in ieee_set.items())
        ...
        logger.info(f"To use, add the following to your kismet_site.conf:\nouifile={out_path}")
    
    def lookup(self, addr: str) -> set:
        # match AA-BB-CC, AA:BB:CC, AABBCC - up to full MAC addr        
        m = re.fullmatch(r'^([0-9A-F]{2})[:-]?([0-9A-F]{2})[:-]?([0-9A-F]{2})(?:[:-]?(?:[0-9A-F]{2})?[:-]?(?:[0-9A-F]{2})?[:-]?(?:[0-9A-F]{2})?)?$', addr.upper())
        if m:
